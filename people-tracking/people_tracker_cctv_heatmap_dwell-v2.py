#!/usr/bin/env python3
"""
People Tracking + HeatMap + Dwell Time
WITH RIGHT-SIDE INFO PANEL (OpenCV UI)
"""
...
def draw_info_panel(height, width, dwell):
    panel = np.zeros((height, width, 3), dtype=np.uint8)

    cv2.putText(panel, "DWELL TIME (sec)", (10, 30),
                cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)

    y = 70
    for tid, data in sorted(dwell.items()):
        text = f"ID {tid} : {data['total']:.1f}"
    ...
    while True:
        ...
        for t in tracks:
            ...
            if tid not in dwell:
                ...
                px, py = dwell[tid]["pos"]
                if math.hypot(cx - px, cy - py) <= dwell_radius:
            ...
            cv2.putText(vis, f"ID {tid}", (cx + 6, cy),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)

        panel = draw_info_panel(vis.shape[0], PANEL_WIDTH, dwell)
        output = np.hstack((vis, panel))

        cv2.imshow("People Tracking + Heatmap + Analytics", output)
