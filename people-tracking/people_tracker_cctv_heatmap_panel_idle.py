#!/usr/bin/env python3
"""
People Tracking + HeatMap + Dwell + Idle/Moving Status
WITH RIGHT-SIDE INFO PANEL
"""
...
def draw_info_panel(height, width, data):
    panel = np.zeros((height, width, 3), dtype=np.uint8)

    cv2.putText(panel, "ID | STATE | DWELL (s)", (10, 30),
                cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 255), 2)

    y = 70
    for tid, d in sorted(data.items()):
        color = (0, 255, 0) if d["state"] == "MOVING" else (0, 0, 255)
        text = f"{tid:2d} | {d['state']:^6} | {d['total']:.1f}"
...
def main():
    ...
    while True:
        ...
        for t in tracks:
            ...
            color = (0, 255, 0) if p["state"] == "MOVING" else (0, 0, 255)
            cv2.circle(vis, (cx, cy), 5, color, -1)
            cv2.putText(vis, f"ID {tid} {p['state']}",
                        (cx + 6, cy),
                        cv2.FONT_HERSHEY_SIMPLEX,
                        0.5, color, 2)

        panel = draw_info_panel(vis.shape[0], PANEL_WIDTH, people)
        output = np.hstack((vis, panel))

        cv2.imshow("People Tracking + Heatmap + Idle Detection", output)
