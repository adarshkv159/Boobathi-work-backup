...
def load_models():
    ...
    try:
        ...
        # Load models
        detector = YoloFace("yoloface_int8.tflite", "/usr/lib/libvx_delegate.so")
        recognizer = Facenet("facenet_512_int_quantized.tflite", "/usr/lib/libvx_delegate.so")
...
class LightweightTTS:
    """Lightweight TTS using espeak-ng"""
        ...
        def run_tts():
            try:
                if self.tts_device != DEFAULT_TTS_DEVICE:
                    tts_proc = subprocess.Popen(['espeak-ng', text, '--stdout'],stdout=subprocess.PIPE,stderr=subprocess.DEVNULL)
                    subprocess.run(['aplay', '-D', self.tts_device],stdin=tts_proc.stdout,stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL)
                else:
                    subprocess.run(['espeak-ng', text],stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL,timeout=5)
            except Exception as e:
...
# Add this to your app_state initialization
app_state['announced_names'] = set()
            ...
            if detector and recognizer and not app_state['capturing_samples']:
                try:
                    ...
                    for box in boxes:
                        ...
                        if name != "Unknown" and name not in app_state['announced_names']:
                            tts.say(name)
                            app_state['announced_names'].add(name)
                except Exception as e:
...
def handle_select_face_sample(data):
    ...
    time.sleep(1)
    tts.say(f"Hello {name}, nice to meet you")
