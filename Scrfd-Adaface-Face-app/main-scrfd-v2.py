...
import tflite_runtime.interpreter as tflite  # only tflite_runtime
...
# ---------------------------
# SCRFD float32 detector (from your float32-tflite.py) [file:94]
# ---------------------------
...
class SCRFD:
    ...
    def _init_vars(self):
        ...
        # mapping from your float32-tflite.py [file:94]
        self.output_map = {
...
class FaceRecognitionTFLiteInt8:
    def __init__(self, tflite_path: str, num_threads: int = 2, delegate=None, rgb: bool = True):
        ...
        shp = self.inp["shape"]  # [1,112,112,3]
        self.in_h = int(shp[1])
        self.in_w = int(shp[2])
    ...
    def __call__(self, face_bgr_112: np.ndarray) -> np.ndarray:
        ...
        # match original ONNX preprocessing (important) [file:3]
...
# ---------------------------
# DB (multi-template per person)
# ---------------------------
def db_empty():
    return {
        "person_names": [],   # list[str], index = person_id
        "labels": np.zeros((0,), dtype=np.int32),        # N
        "embeddings": np.zeros((0, 512), dtype=np.float32),  # N x 512
        "paths": np.array([], dtype=object),             # N
    }
...
def match_identity(db: dict, emb: np.ndarray, threshold: float):
    """
    Returns: (name, best_score, best_template_index) or ("Unknown", best_score, idx)
    Uses MAX over templates (best match).
    """
    if db["embeddings"].shape[0] == 0:
        return "Unknown", None, None

    scores = db["embeddings"] @ emb  # cosine (embeddings are normalized)
...
def compute_embedding_from_bgr(img_bgr, detector: SCRFD, recognizer: FaceRecognitionTFLiteInt8, det_thresh: float):
    det, kpss = detector.detect(img_bgr, thresh=det_thresh, input_size=detector.input_size, max_num=10)
    if kpss is None or len(kpss) == 0:
        return None
    kpt = kpss[0].astype(np.float32)
    face = norm_crop_image(img_bgr, kpt, 112)
    emb = recognizer(face).astype(np.float32)
...
def capture_dataset_images(dataset_dir: str, person_name: str, cam: int,
    ...
    """
    Captures raw frames (full image) into dataset/person_name/.
    Use different distances (close/long) and press SPACE multiple times.
    """
    ...
    print("Capture dataset images")
    print("Controls: SPACE=save image, q=quit")
    ...
    while True:
        ...
        det, kpss = detector.detect(frame, thresh=det_thresh, input_size=detector.input_size, max_num=1)
        view = frame.copy()
...
def recognize_live(db_path: str, cam: int,
                   detector: SCRFD, recognizer: FaceRecognitionTFLiteInt8,
                   threshold: float, det_thresh: float):
    db = db_load(db_path)
    if db["embeddings"].shape[0] == 0:
        raise RuntimeError(f"DB is empty: {db_path}. Build from dataset first.")

    cap = cv.VideoCapture(cam)
    if not cap.isOpened():
        raise RuntimeError(f"Cannot open camera {cam}")

    print("Recognize live: press q to quit")
    ...
    while True:
        ...
        if kpss is not None and len(kpss) > 0:
            ...
            label, score, best_idx = match_identity(db, emb, threshold)

            x1, y1, x2, y2 = det[0][:4].astype(int)
            cv.rectangle(frame, (x1, y1), (x2, y2), (0, 255, 0), 2)

            if best_idx is not None:
                tmpl_path = str(db["paths"][best_idx])
                cv.putText(frame, os.path.basename(tmpl_path), (20, 75),
                           cv.FONT_HERSHEY_SIMPLEX, 0.7, (50, 255, 50), 2)

        txt = label if score is None else f"{label}  sim={score:.3f} thr={threshold:.2f}"
        cv.putText(frame, txt, (20, 40),
                   cv.FONT_HERSHEY_SIMPLEX, 1.0, (50, 255, 50), 2)
...
def main():
    ...
    ap.add_argument("--scrfd_npu", action="store_true", help="Run SCRFD on VX delegate (else CPU)")
    ap.add_argument("--rec_delegate", default="", help="VX delegate .so for recognition (CPU if empty)")
    ...
    if args.cmd == "capture_dataset":
        ...
        recognize_live(args.db, args.cam, detector, recognizer,
                       threshold=args.threshold, det_thresh=args.det_thresh)
