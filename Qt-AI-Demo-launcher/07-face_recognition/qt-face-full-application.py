...
class FaceRecognitionThread(QThread):
    ...
    def set_acceleration(self, use_npu, delegate_path):
        """Set NPU acceleration settings"""
    ...
    def initialize_models(self):
        ...
        try:
            ...
            if self.use_npu and self.delegate_path and os.path.exists(self.delegate_path):
                try:
                    self.detector = YoloFace("yoloface_int8.tflite", self.delegate_path)
                    self.message_signal.emit("✓ Face detection loaded on NPU", "success")
                except Exception as e:
                    self.message_signal.emit(f"NPU loading failed: {e}, using CPU", "warning")
                    self.detector = YoloFace("yoloface_int8.tflite", "")
            else:
                self.detector = YoloFace("yoloface_int8.tflite", "")
                self.message_signal.emit("✓ Face detection loaded on CPU", "info")
            ...
            if self.use_npu and self.delegate_path and os.path.exists(self.delegate_path):
                try:
                    ...
                    self.message_signal.emit("✓ Face recognition loaded on NPU", "success")
                except Exception as e:
                    self.message_signal.emit(f"NPU loading failed: {e}, using CPU", "warning")
                    self.recognizer = Facenet("facenet_512_int_quantized.tflite", "")
            else:
                self.recognizer = Facenet("facenet_512_int_quantized.tflite", "")
                self.message_signal.emit("✓ Face recognition loaded on CPU", "info")
            ...
            self.message_signal.emit(f"All models loaded successfully on {mode}!", "success")
            return True
    ...
    def run(self):
        ...
        self.running = True
        mode = "NPU" if self.use_npu else "CPU"
        self.status_signal.emit(f"Ready ({mode})", "#00FF00")
...
class FaceRecognitionGUI(QMainWindow):
    ...
    def create_right_panel(self):
        ...
        self.cpu_radio = QRadioButton("CPU Mode")
        self.cpu_radio.setChecked(True)
        self.npu_radio = QRadioButton("NPU Mode (VX Delegate)")
    ...
    def check_delegate(self):
        ...
        if not delegate_path:
            self.add_message("No delegate path specified", "warning")
            return
        
        if os.path.exists(delegate_path):
            file_size = os.path.getsize(delegate_path) / (1024 * 1024)
            self.add_message(f"✓ Delegate found: {delegate_path} ({file_size:.2f} MB)", "success")
        else:
            self.add_message(f"✗ Delegate NOT found: {delegate_path}", "error")
            self.add_message("Common paths: /usr/lib/libvx_delegate.so or /usr/lib64/libvx_delegate.so", "info")
    ...
    def start_system(self):
        ...
        if use_npu and not os.path.exists(delegate_path):
            self.add_message(f"NPU delegate not found: {delegate_path}", "error")
            self.add_message("Switching to CPU mode", "warning")
            use_npu = False
        
        # Set acceleration
        self.recognition_thread.set_acceleration(use_npu, delegate_path)
        
        mode = "NPU" if use_npu else "CPU"
        self.add_message(f"Starting face recognition system in {mode} mode...", "info")
