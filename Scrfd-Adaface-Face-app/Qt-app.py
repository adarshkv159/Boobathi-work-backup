...
from PySide6.QtWidgets import (
    ...
    QSpinBox, QDoubleSpinBox, QCheckBox, QFileDialog, QTabWidget,
...
# ---------------------------
# Recognition int8 (from your file)
# ---------------------------
...
# ---------------------------
# Dataset helpers (from your file)
# ---------------------------
    ...
    def run(self):
        try:
            ...
            while self._running:
                ...
                # Small sleep via waitKey-like pacing using OpenCV timing
                # (keeps CPU lower and UI responsive)
                cv.waitKey(1)
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("SCRFD Face App (PySide6)")
    ...
    def _build_ui(self):
        ...
        self.ed_vx_delegate = QLineEdit(self.cfg.vx_delegate)
        self.cb_scrfd_npu = QCheckBox("Use SCRFD NPU (vx delegate)")
        self.cb_scrfd_npu.setChecked(self.cfg.scrfd_npu)
        ...
        form.addRow("VX delegate path:", self.ed_vx_delegate)
        form.addRow("", self.cb_scrfd_npu)
        ...
        self.sp_threshold.setValue(0.55)
    ...
    def load_models(self):
        try:
            ...
            self.cfg.vx_delegate = self.ed_vx_delegate.text().strip()
            self.cfg.scrfd_npu = self.cb_scrfd_npu.isChecked()
            ...
            self.append_log("Loading SCRFD...")
            self.detector = SCRFD(
                model_file=self.cfg.scrfd_model,
                nms_thresh=0.4,
                use_npu=self.cfg.scrfd_npu,
                delegate_path=self.cfg.vx_delegate,
            ...
            self.append_log(f"SCRFD backend: {'NPU' if self.cfg.scrfd_npu else 'CPU'}")
            self.append_log(f"SCRFD input  : {self.detector.input_size} (W,H)")
            self.append_log(f"REC backend  : {self.cfg.rec_delegate if rec_delegate else 'CPU'}")
            QMessageBox.information(self, "OK", "Models loaded.")
        except Exception as e:
    ...
    def on_frame(self, img: QImage):
        # Show in the active tab’s video label.
        ...
        if target is None:
            # If not on capture/recognize, still show something on capture label
            target = self.cap_video

        pix = QPixmap.fromImage(img)
        target.setPixmap(pix.scaled(target.size(), Qt.AspectRatioMode.KeepAspectRatio, Qt.TransformationMode.SmoothTransformation))
    ...
    # Safety: stop camera while deleting files
        if self.camera_worker.isRunning():
            self.stop_camera()
