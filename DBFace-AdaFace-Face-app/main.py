...
def camera_demo(use_npu=True):
    delegate_path = '/usr/lib/libvx_delegate.so'
    
    if use_npu and os.path.exists(delegate_path):
        print(f"Loading NPU delegate from: {delegate_path}")
        try:
            # Load VX delegate with options
            ...
            ext_delegate = [tflite.load_delegate(delegate_path, ext_delegate_options)]
            
            interpreter = tflite.Interpreter(
                model_path='model_full_integer_quant.tflite',
        ...
        if not os.path.exists(delegate_path):
            print(f"⚠ NPU delegate not found at: {delegate_path}")
    ...
    while True:
        ...
        # Detect with adjusted threshold
        objs = detect(interpreter, input_details, output_details, img, threshold=0.62, nms_iou=0.3)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--use-npu', action='store_true', default=True,
                        help='Use NPU acceleration (default: True)')
    parser.add_argument('--cpu-only', action='store_true', default=False,
                        help='Force CPU-only inference')
    args = parser.parse_args()
    
    use_npu = args.use_npu and not args.cpu_only
    camera_demo(use_npu=use_npu)
