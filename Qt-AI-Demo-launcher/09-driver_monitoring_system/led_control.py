...
# Initialize LEDs (turn off all at start)
turn_off_all_leds()
...
def main(image):
    ...
    for i, (mesh_landmark, r_vec, t_vec) in enumerate(zip(mesh_landmarks_inverse, r_vecs, t_vecs)):
        ...
        # === Eye Close Detection (with 0.5-second trigger) ===
...
try:
    ...
    # Cleanup: Turn off all LEDs when exiting
    turn_off_all_leds()
    time.sleep(2)
    cap.release()
    cv2.destroyAllWindows()
    print("LEDs turned off and resources cleaned up.")
